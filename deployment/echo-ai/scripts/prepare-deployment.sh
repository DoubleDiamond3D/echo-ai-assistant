...
create_deployment_package() {
    ...
    # Copy Echo files (excluding deployment dir)
    log "Copying Echo files..."
    mkdir -p "$DEPLOY_DIR/echo-ai"
    
    # Copy files individually to avoid directory conflicts
    for item in "$REPO_DIR"/*; do
        if [[ "$(basename "$item")" != "deployment" ]]; then
            cp -r "$item" "$DEPLOY_DIR/echo-ai/"
        fi
    done
